...
namespace PWT::UI::FANS {
    FanControlTab::FanControlTab(const PWTS::DeviceInfoPacket &packet) {
        for (const auto &[fanID, label]: packet.fanLabels.asKeyValueRange()) {
            FanControlGBox *fanControl = new FanControlGBox(packet.features.fans[fanID], label, fanID);
    ...
    void FanControlTab::refreshTab(const PWTS::DaemonPacket &packet) {
        for (FanControlGBox *gbox: fanControlList)
            gbox->setData(packet);
    }

    void FanControlTab::setDataForPacket(PWTS::ClientPacket &packet) const {
        for (const FanControlGBox *gbox: fanControlList)
            packet.fanData.insert(gbox->getFanID(), gbox->getData());
    }
}
