...
namespace PWT::UI::WIN {
	...
	private:
		QComboBox *schemeSelect = nullptr;
		QString prevSelected;

	public:
		SchemeSelectInput();

		[[nodiscard]] QString getCurrentScheme() const { return schemeSelect->currentText(); }
		[[nodiscard]] bool hasSchemes() const { return schemeSelect->count(); }
		void deleteCurrentScheme() const { schemeSelect->removeItem(schemeSelect->currentIndex()); }
