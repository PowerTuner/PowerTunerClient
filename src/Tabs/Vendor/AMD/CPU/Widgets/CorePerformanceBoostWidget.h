...
namespace PWT::UI::AMD {
    class CorePerformanceBoostWidget final: public GridWidget {
    private:
        QList<QComboBox *> comboList;

    public:
        explicit CorePerformanceBoostWidget(int threadCount);

        void setData(const PWTS::DaemonPacket &packet);
        void setDataForPacket(const PWTS::ClientPacket &packet) const;
        void setCorePerformanceBoostAll(int idx) const;
    };
}
