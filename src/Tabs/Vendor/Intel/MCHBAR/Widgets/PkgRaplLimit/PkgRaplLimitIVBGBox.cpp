 ...
 */
#include "PkgRaplLimitIVBGBox.h"
#include "pwtClientCommon/UILogger.h"
#include "../../../../../Include/FlowLayout.h"

namespace PWT::UI::INTEL {
    PkgRaplLimitIVBGBox::PkgRaplLimitIVBGBox() {
        ...
        QObject::connect(pl1Enable, &QCheckBox::checkStateChanged, this, &PkgRaplLimitIVBGBox::onPl1EnableChanged);
        QObject::connect(pl2Enable, &QCheckBox::checkStateChanged, this, &PkgRaplLimitIVBGBox::onPl2EnableChanged);
    }

    void PkgRaplLimitIVBGBox::setRanges(const PWTS::MinMax &range) const {
        pl1->setRange(range.min, range.max);
        pl2->setRange(range.min, range.max);
    }

    void PkgRaplLimitIVBGBox::setData(const PWTS::DaemonPacket &packet) {
    ...
    void PkgRaplLimitIVBGBox::setDataForPacket(const PWTS::ClientPacket &packet) const {
        if (!isEnabled())
            return;
    ...
    void PkgRaplLimitIVBGBox::onPl1EnableChanged(const Qt::CheckState state) const {
        pl1->setEnabled(state == Qt::Checked);
    }

    void PkgRaplLimitIVBGBox::onPl2EnableChanged(const Qt::CheckState state) const {
