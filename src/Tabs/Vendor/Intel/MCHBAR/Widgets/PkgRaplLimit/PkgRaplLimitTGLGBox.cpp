 ...
 */
#include "PkgRaplLimitTGLGBox.h"
#include "pwtClientCommon/UILogger.h"
#include "../../../../../Include/FlowLayout.h"

namespace PWT::UI::INTEL {
    PkgRaplLimitTGLGBox::PkgRaplLimitTGLGBox() {
        ...
        QObject::connect(pl1Enable, &QCheckBox::checkStateChanged, this, &PkgRaplLimitTGLGBox::onPl1EnableChanged);
        QObject::connect(pl2Enable, &QCheckBox::checkStateChanged, this, &PkgRaplLimitTGLGBox::onPl2EnableChanged);
    }

    void PkgRaplLimitTGLGBox::setRanges(const PWTS::MinMax &range) const {
        pl1->setRange(range.min, range.max);
        pl2->setRange(range.min, range.max);
    }

    void PkgRaplLimitTGLGBox::setData(const PWTS::DaemonPacket &packet) {
    ...
    void PkgRaplLimitTGLGBox::setDataForPacket(const PWTS::ClientPacket &packet) const {
        if (!isEnabled())
            return;
    ...
    void PkgRaplLimitTGLGBox::onPl1EnableChanged(const Qt::CheckState state) const {
        pl1->setEnabled(state == Qt::Checked);
    }

    void PkgRaplLimitTGLGBox::onPl2EnableChanged(const Qt::CheckState state) const {
